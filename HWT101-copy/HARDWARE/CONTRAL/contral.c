...
void uart_HW101_save(void)
{
	...
	uint8_t yawl=0x00;
	uint8_t yawh=0x00;
	yaw2 = Serial3_RXPacket[1];   //接收陀螺仪向串口发送的信息,帧头为0X55，从帧头之后的开始读		
	if(yaw2==0X53)				 //此时读到了陀螺仪发回的角度信息
	{
	yawl = Serial3_RXPacket[6];		//把角度的低八位和高八位取出来，准备给上位机发
	yawh = Serial3_RXPacket[7];
	Serial3_HW101_Angle[0] = yawl;	//把角度低八位和高八位拼在一起，放到专门的数组里
	Serial3_HW101_Angle[1] = yawh;		
	}
...
UART3_SendArray(unlock_register, sizeof(unlock_register));
UART3_SendArray(reset_z_axis, sizeof(reset_z_axis));
UART3_SendArray(save_settings, sizeof(save_settings));
UART3_SendArray(restart_device, sizeof(restart_device));
}
...
void claw_down(void)
{
	...
	distance1=9600+1500;
	stepPosition1=0;
	MSD_Move1(11100,16,16,32);  // 9600+1500            3600+1500= 5100
	while(1)
	{
		if(stepPosition1 == distance1)
		{
