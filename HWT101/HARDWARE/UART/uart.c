...
uint8_t Serial_Flag3;
uint8_t Serial_Flag4;
...
uint8_t Serial4_TXPacket[4];
uint8_t Serial4_RXPacket[11];
uint8_t Rx4Packet[11];
uint8_t Serial4_HW101_Angle[2];
...
/******************************************************************************
PC10  TX4(串口4只有这一个TX脚)
PC11  RX4(串口4只有这一个RX脚)
******************************************************************************/
/********************************
函数功能 : 串口4的初始化
映射
...
{
	//	static uint8_t Rx4Packet[11];
	static uint8_t RX4State=0;
	static uint8_t pRx4Packet=0;
	if(USART_GetITStatus(UART4,USART_IT_RXNE) == SET)
	{
	uint8_t Rx4Data = USART_ReceiveData(UART4);

	  if(RX4State == 0)
		{
		if(Rx4Data == 0x55)//陀螺仪报头
			{
					Rx4Packet[0] = Rx4Data;
					RX4State =1;
					pRx4Packet=1;
//					pRx4Packet=0;
	...
	{
			Rx4Packet[pRx4Packet++] = Rx4Data;
			if(pRx4Packet==2&&Rx4Packet[1]!=0x53&&Rx4Packet[1]!=0x52)
		...
		else if(pRx4Packet == 11)
			{
			ParseData(Rx4Packet,11);
			pRx4Packet = 0;
				RX4State = 0;
			}
...
{
	UART4_SendBits(0XFF);
	UART4_SendArray(Serial4_RXPacket,4);
    UART4_SendBits(0XFE);
}
